...
- [ ] **Day 1 — Scaffold, data, EDA.** Repo/GitHub setup, data ingestion, chronological train/test split
      (no leakage), compact EDA notebook.
- [ ] **Day 2 — Baselines.** Feature pipeline, Logistic Regression baseline, RandomForest/XGBoost baseline,
      MLflow tracking wired in.
- [ ] **Day 3 — Autoencoder + imbalance handling.** Unsupervised autoencoder (reconstruction error), honest
      writeup of why it over/underperforms, SMOTE vs. class-weighting comparison.
- [ ] **Day 4 — Cost-sensitive decision engine.** Illustrative FN/FP costs, threshold optimizer, cost-vs-
      threshold curve, cost-ratio sensitivity sweep.
- [ ] **Day 5 — Evaluation + interpretability.** Precision/recall/F1, PR-AUC/ROC-AUC, confusion matrix,
